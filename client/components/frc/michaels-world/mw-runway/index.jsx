...
import { RunwaySection } from './components';
...
const getGlobalOptions = (globalOpt, compId) => {
  const globalOptions = {
    ...
    background: {
       ...
       color: (globalOpt.prod_grid_only && compId === "grid" || globalOpt.banner_only && compId === "banner" || globalOpt.entire_page)
...
const getComponent = (data, generator) => {
  ...
  sections.forEach((section, index) => {
    ...
    if (section.componentId) {
      html = <RunwaySection key={index}>{generator.renderJSONComponents([section])}</RunwaySection>;
      component.push(html);
    } else {
      const compSections = renderComponent(section);
      compSections.forEach((comp, idx) => {
        html = <RunwaySection key={`section_${idx}`}>{generator.renderJSONComponents([comp])}</RunwaySection>;
        component.push(html);
      });
    }
  });
...
class MwRunway extends React.Component {
  constructor(props) {
    super(props);
